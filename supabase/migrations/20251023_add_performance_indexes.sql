-- ============================================
-- ОПТИМИЗАЦИЯ: Добавление индексов для производительности
-- Дата: 23 октября 2025
-- ============================================

-- Индексы для таблицы tenders (самые частые запросы)
...
-- Составной индекс для частого запроса "тендеры пользователя по статусу"
CREATE INDEX IF NOT EXISTS idx_tenders_user_status ON public.tenders(user_id, status);

-- Составной индекс для "расходы тендера пользователя"
CREATE INDEX IF NOT EXISTS idx_expenses_user_tender ON public.expenses(user_id, tender_id);

-- ============================================
-- Ожидаемый результат:
-- - Запросы к БД: 200-500ms → <50ms
-- - Фильтрация по статусу: мгновенная
