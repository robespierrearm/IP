...
export async function POST(request: NextRequest) {
  try {
    ...
    if (url && !html) {
      try {
        console.log('🌐 Пытаюсь загрузить страницу:', url);
        ...
        console.log('✅ Страница загружена, размер:', pageContent.length);
      } catch (error: any) {
        console.error('❌ Ошибка загрузки страницы:', error.message);
        
        // Определяем тип ошибки и даём конкретный совет
        let errorMessage = 'Не удалось загрузить страницу по ссылке.';
        let hint = 'Откройте страницу тендера в браузере → Нажмите Ctrl+U → Скопируйте весь код → Вставьте в поле "HTML код"';
        
        if (error.message.includes('timeout') || error.message.includes('ETIMEDOUT')) {
          errorMessage = '⏱️ Сайт не отвечает или слишком медленный.';
        } else if (error.message.includes('403') || error.message.includes('401')) {
          errorMessage = '🔐 Сайт требует авторизацию или блокирует автоматические запросы.';
        } else if (error.message.includes('TLS') || error.message.includes('SSL')) {
          errorMessage = '🛡️ Сайт блокирует автоматические запросы (защита от ботов).';
          hint = '💡 Многие тендерные площадки (Сбербанк-АСТ, ЕТС и др.) блокируют парсеры. Используйте метод "По HTML": откройте страницу в браузере → Ctrl+U → Скопируйте код → Вставьте в парсер';
        }
